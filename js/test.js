...
// ーーーーーーーーーーーーー文字数をカウントする関数(countTexts)
// ーーーーーーーーーーーーーリスト要素とそれに紐づくローカルストレージを生成する関数(addMemo)
// ーーーーーーーーーーーーー一意のリスト要素をクリックしたときにselectlistを発動する（選択させる）
// ーーーーーーーーーーーーーulの一番最初にリスト要素を追加する
// ーーーーーーーーーーーーー選択する（CSSの付与とテキストの表示）関数(selectList)
// ーーーーーーーーーーーーー書き込まれたタイトルとテキストをローカルストレージに保存する処理
// ーーーーーーーーーーーーーローカルストレージのタイトルデータを取得してリスト要素内のtitleTextに反映させる
// ーーーーーーーーーーーーータイムスタンプを配列化させてソートする処理
// ーーーーーーーーーーーーーソートされたデータを元にロード時にリスト要素として表示させる処理
// ーーーーーーーーーーーーーsvgアイコンをクリックすることでモーダルを表示させる処理
// ーーーーーーーーーーーーーモーダル内で削除ボタンを押すとローカルストレージのデータと表層のリスト要素を削除する処理とその後の処理
// ーーーーーーーーーーーーー作成ボタンを押したらaddMemoが生成される処理
// ーーーーーーーーーーーーー常にどれかの要素を選択させておくための処理(firstListSelect)
// ーーーーーーーーーーーーーローカルストレージにデータが一つもない場合はaddMemoを、一つ以上ある場合はloadContentsとfirstListSelectを発動
// ーーーーーーーーーーーーー全般的なローカルストレージからのデータの取得方法とクラスやidからのデータの取得
